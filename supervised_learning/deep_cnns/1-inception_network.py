...
def inception_network():
    ...
    x = K.layers.Conv2D(64, kernel_size=7, strides=2, padding='same', activation='relu')(input_layer)
    x = K.layers.MaxPooling2D(pool_size=3, strides=2, padding='same')(x)

    x = K.layers.Conv2D(64, kernel_size=1, padding='same', activation='relu')(x)
    x = K.layers.Conv2D(192, kernel_size=3, padding='same', activation='relu')(x)
    x = K.layers.MaxPooling2D(pool_size=3, strides=2, padding='same')(x)
