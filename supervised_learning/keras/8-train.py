...
Module train_model contains function:
    train_model(network, data, labels, batch_size, epochs, validation_data=None, early_stopping=False,
                patience=0, learning_rate_decay=False, alpha=0.1, decay_rate=1, save_best=False, filepath=None,
...
def train_model(network, data, labels, batch_size, epochs, validation_data=None, early_stopping=False,
    ...
    """
    Trains a model using mini-batch gradient descent with optional validation data, early stopping, learning rate decay,
    ...
    Args:
        ...
        save_best: A boolean that indicates whether to save the best iteration of the model.
        filepath: The file path where the model should be saved if save_best is True.
    ...
    # Add EarlyStopping callback if early_stopping is enabled and validation data exists
    ...
    if save_best and validation_data is not None and filepath is not None:
        checkpoint_callback = K.callbacks.ModelCheckpoint(filepath=filepath, monitor='val_loss',
